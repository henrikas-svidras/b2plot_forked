...
def flat_corr_matrix(df, pdf=None, tight=False, col_numbers=False, labels=None, labelsize=14, size=12, n_labels=3, 
                     fontsize=22, draw_cbar=False,  tick_label_rotation=45, formatter='%.2e',label_distance = 10,label_rotation=None):
        ...
        for j, ax in zip(range(n_vars), row):
            if i == n_vars - 1:
                ...
                if col_numbers:
                    ...
                    ax.set_xlabel(labels[j], fontsize=fontsize, labelpad = label_distance-20,
                    rotation=0 if not label_rotation else label_rotation)
            if j == 0:
                ...
                if col_numbers:
                    ...
                    ax.set_ylabel(labels[i], fontsize=fontsize, labelpad = label_distance,
                    rotation=90 if not label_rotation else 90-label_rotation)
